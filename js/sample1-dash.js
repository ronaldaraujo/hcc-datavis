...
d3.csv("data/sample1-datafile.csv", function (data) {
  ...
  var year_total = dateDim.group().reduceSum(function (d) {
    return d.value;
  });
  var delhi = dateDim.group().reduceSum(function (d) {
    if (d.city === "Delhi") {
      return d.value;
    } else {
      return 0;
  ...
  var mumbai = dateDim.group().reduceSum(function (d) {
    if (d.city === "Mumbai") {
      return d.value;
    } else {
      return 0;
  ...
  var hyderabad = dateDim.group().reduceSum(function (d) {
    if (d.city === "Hyderabad") {
      return d.value;
    } else {
      return 0;
  ...
  var chennai = dateDim.group().reduceSum(function (d) {
    if (d.city === "Chennai") {
      return d.value;
    } else {
      return 0;
  ...
  var kolkata = dateDim.group().reduceSum(function (d) {
    if (d.city === "Kolkata") {
      return d.value;
    } else {
      return 0;
  ...
  var pune = dateDim.group().reduceSum(function (d) {
    if (d.city === "Pune") {
      return d.value;
    } else {
      return 0;
  ...
  moveChart.yAxis().tickFormat(function (v) {
    return v / 1000 + "M";
  });
  moveChart
    .renderArea(true)
  ...
  yearChart.yAxis().tickFormat(function (v) {
    return v / 1000 + "M";
  });
  yearChart
    .width(500)
  ...
  datatable
    ...
    .group(function (d) {
      return "Vehical registrations in million";
    })
    ...
    .columns([
      ...
      function (d) {
        return d.value.Delhi;
      },
      function (d) {
        return d.value.Mumbai;
      },
      function (d) {
        return d.value.Chennai;
      },
      function (d) {
        return d.value.Kolkata;
      },
      function (d) {
        return d.value.Pune;
      },
      function (d) {
        return d.value.Hyderabad;
      },
      function (d) {
        return (
          d.value.Delhi +
